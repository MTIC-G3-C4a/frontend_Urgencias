...
const AllEnfermedades = () => {
    ...
    const { isAuth, setEditandoEnfermedad, enfermedades } = useContext(contextAuth);
    const history = useHistory();
    const [loadingDelete, setLoadingDelete] = useState(false);

    //editar enfermedad
    const handleEditEnfermedad = (enfermedad) => {
        console.log("editando " + enfermedad?.nombre);
            ...
            {enfermedades?.getAllEnfermedades.map((enfermedad) => (
                <div className="paciente" key={enfermedad.nombre}>
                    ...
                    <div className="container-btns-paciente">
                        ...
                        <button onClick={()=>handleEditEnfermedad(enfermedad)} className="btn-editar">
                            Editar
                        </button>
                        <button
